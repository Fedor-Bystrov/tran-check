...
#### Запуск
- Создать `config.properties` рядом с jar файлом. В нем указать `csvFile=<полный путь до csv с транзакциями>`.
Пример:
```
csvFile=./ptxs.csv
```

- Выполнить `java -jar tran-check-0.0.1-SNAPSHOT.jar`. 
Полезно смотреть в лог, нет ли `ERROR`. 
- В случае успеха в логе будет строка  `Creating report file at <путь до репорта>`.
